...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Wrap all options
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "infotext" ) && ( $value['type'] != "info" ) && ( $value['type'] != "adstwofifty" ) && ( $value['type'] != "innertabopen" ) && ( $value['type'] != "innertabclose" ) && ( $value['type'] != "tabheading" ) && ( $value['type'] != "groupcontaineropen" ) && ( $value['type'] != "groupcontainerclose" ) && ( $value['type'] != "accordiancont_open" ) && ( $value['type'] != "accordiancont_close" ) && ( $value['type'] != "groupcontaineropen_accordian" ) && ( $value['type'] != "groupcontainerclose_accordian" ) ) {
		...
		case "accordiancont_open":

			$output .= '<div id="'.  esc_attr( $value['id'] ).'">';

			break;			
			
		case "accordiancont_close":

			$output .= '</div>';

			break;			
			
		case "groupcontaineropen_accordian":

			$output .= '<h3>' . esc_html( $value['name'] ) . '</h3><div><div class="groupcontainer accordian_border_top_none">';
			
			break;
			
		case "groupcontainerclose_accordian":
